...
export function chromeWebStoreConfigError(value) {
  ...
  if (!/^[A-Za-z0-9_-]{8,128}$/.test(config.publisherId)) {
    return 'Chrome Web Store publisher ID is missing or invalid. Configure it in Settings → Skills.';
  }
  if (!/^[a-p]{32}$/.test(config.itemId)) {
    return 'Chrome Web Store item ID must be the 32-character extension ID (letters a–p). Configure it in Settings → Skills.';
  }
  return '';
}
...
export function chromeWebStoreEndpoint(toolName, config) {
  ...
  if (toolName === 'chrome_web_store_publish') {
...
export function isTrustedChromeWebStoreSkillTool(tool) {
  return !!tool
    && tool.kind === 'chromeWebStore'
    && tool.skillId === CHROME_WEB_STORE_SKILL_ID
    && tool.sourceType === 'built-in'
    && tool.sourceUrl === CHROME_WEB_STORE_SKILL_PATH
...
export async function executeChromeWebStoreSkillTool(tool, args = {}, opts = {}) {
  if (!isTrustedChromeWebStoreSkillTool(tool)) {
    return { success: false, dispatched: false, noDispatch: true, error: 'Untrusted Chrome Web Store tool declaration rejected.' };
  }
  ...
  if (tool.name === 'chrome_web_store_upload') {
    ...
    if (!packageInfo?.base64 || !packageInfo?.name) {
      return { success: false, dispatched: false, noDispatch: true, error: 'No release ZIP is selected. Choose one in Settings → Skills before uploading.' };
    }
    if (!/\.zip$/i.test(packageInfo.name) || Number(packageInfo.size) > CHROME_WEB_STORE_MAX_PACKAGE_BYTES) {
      return { success: false, dispatched: false, noDispatch: true, error: 'The selected release package is not a valid supported ZIP.' };
    }
    init = {
      method: 'POST',
      headers: { Accept: 'application/json', 'Content-Type': 'application/zip' },
      body: decodeBase64(packageInfo.base64),
    };
    ...
    if (requestedPublishType !== 'default' && requestedPublishType !== 'staged') {
      return {
        success: false,
        dispatched: false,
        noDispatch: true,
    ...
    if (percentage != null && (!Number.isInteger(percentage) || percentage < 0 || percentage > 100)) {
      return { success: false, dispatched: false, noDispatch: true, error: 'deploy_percentage must be an integer from 0 to 100.' };
    }
    init = {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        ...(percentage == null ? {} : { deployInfos: [{ deployPercentage: percentage }] }),
      }),
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        ...(consequential ? { outcomeUnknown: response.status >= 500 } : { noDispatch: true }),
        status: response.status,
        error: responseError(response, payload),
      };
    }
    return {
      success: true,
      dispatched: consequential,
      ...(tool.name === 'chrome_web_store_status' ? { observed: true } : {}),
      ...(packageInfo ? {
        package: {
          name: packageInfo.name,
          size: Number(packageInfo.size) || 0,
          sha256: clean(packageInfo.sha256, 128),
        },
      } : {}),
      result: payload,
    };
...
export async function startChromeWebStoreOAuth(config) {
  ...
  return startSubscriptionOAuth(
    CHROME_WEB_STORE_OAUTH_PROVIDER,
    normalized.oauthClientId,
    normalized.oauthClientSecret,
  );
}

export function getChromeWebStoreOAuthStatus() {
  return getSubscriptionStatus(CHROME_WEB_STORE_OAUTH_PROVIDER);
}

export function signOutChromeWebStoreOAuth() {
  return signOutSubscription(CHROME_WEB_STORE_OAUTH_PROVIDER);
}
