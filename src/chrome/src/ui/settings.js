...
document.getElementById('btn-profile-sync-auth')?.addEventListener('click', async () => {
  ...
  try { profileSyncChallenge = await profileSyncAction('profile_sync_auth_start', { email }); showProfileSyncResult(true, 'Check your email, approve Cloud Sync, then return here.');
