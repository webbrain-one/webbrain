...
export default {
  ...
  "st.display.help_improve.desc_html": "允许保留选定的 WebBrain Cloud 交互，并将其用于评估、改进、微调和训练。此选项默认开启。关闭后，未来的 Cloud 交互将不会用于这些用途。WebBrain 绝不会收集本地模型请求或使用您自己的 API 凭据直接发送的请求。<a href=\"https://webbrain.one/privacy\" target=\"_blank\" rel=\"noopener noreferrer\" style=\"color:var(--accent);\">隐私政策 →</a>",
  "st.providers.webbrain_data_use.body": "WebBrain Cloud 包含每日免费用量。帮助改进 WebBrain 默认开启时，选定的 Cloud 对话可能会被保留，并用于评估、改进、微调和训练。可在“常规 → 高级”中关闭，以排除未来的 Cloud 交互。WebBrain 绝不会收集本地模型请求或使用您自己的 API 凭据直接发送的请求。{privacyLink}。如需更多用量，请在 {subscribeLink} 订阅。在 {accountLink} 管理账单。",
