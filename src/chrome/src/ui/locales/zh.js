...
export default {
  ...
  'st.display.plan_before_act.desc': '操作和 Dev 模式在使用工具前始终执行结构化意图检查。尝试（默认）还会生成完整计划，但在简短跟进中可复用最近批准的计划；严格模式每轮都生成完整计划。如果意图或计划在一次修复后仍无效，尝试模式会切换为只读轮次；严格模式会在使用工具前停止。',
