...
export default {
  ...
  'sp.ui_scale.reset': 'リセット',
  'st.display.ui_scale.label': '拡張機能 UI のズーム',
  'st.display.ui_scale.decrease': '拡張機能 UI を縮小',
  'st.display.ui_scale.increase': '拡張機能 UI を拡大',
  'st.display.ui_scale.reset': 'リセット',
  'st.display.ui_scale.desc': 'ページのズームを変更せずに、サイドパネルと独立したチャットウィンドウのサイズを変更します。',
  'st.display.ui_scale.shortcuts': '専用ショートカット: {shortcuts}',
  'st.display.ui_scale.shortcuts_none': '未設定',
  'st.display.ui_scale.manage_shortcuts': 'ショートカットをカスタマイズ',
  ...
  "st.system_one.desc_html": "定期監視と予約タスクの完了を追加検証します。",
  "st.system_one.enabled.label": "Jev を有効にする",
  "st.system_one.api_key.label": "TypeSafe API キー",
  "st.system_one.watch.label": "定期監視を検証",
  "st.system_one.completion.label": "予約タスクの完了を検証",
  "st.system_one.security_html": "タスクと限定的なページ証拠を TypeSafe に送信します。キーはローカルに平文で保存され、利用料金が発生する場合があります。Strict Secret Mode では Jev は無効です。",
  ...
  'sp.btn.settings': '設定',
  "sp.btn.clear": "新しい会話",
  "sp.btn.clear_short": "新しい会話",
  "sp.btn.history_short": "履歴",
  ...
  'tool.set_checked': "チェックボックスを更新中",
  'tool.hover': "ホバー中",
  'tool.drag_drop': "要素をドラッグ中",
  'tool.press_keys': "キーを押下中",
  'tool.press_keys.keys': "{keys} を押下中",
  'tool.find_text': "テキストを検索中",
  'tool.find_text.text': "「{text}」を検索中",
  'tool.upload_file': "ファイルをアップロード中",
  'tool.download_files': "ファイルをダウンロード中",
  'tool.download_media': "メディアをダウンロード中",
  'tool.download_resource': 'ファイルをダウンロード中',
  'tool.check_downloads': "ダウンロードを確認中",
  'tool.read_downloaded_file': "ダウンロードしたファイルを読み取り中",
  'tool.read_frame': "フレームを読み取り中",
  'tool.get_frames': 'フレームを一覧表示中',
  'tool.solve_captcha': "認証を解決中",
  'tool.clarify': "確認を求めています",
  'tool.get_window_info': "ウィンドウを確認中",
  'tool.resize_window': "ウィンドウサイズを変更中",
  'tool.inspect_viewport': "表示領域を確認中",
  'tool.scratchpad_write': "メモを更新中",
  'tool.progress_update': "進捗を更新中",
  'tool.progress_read': "進捗を確認中",
  'tool.verify_form': "フォームを確認中",
  'tool.chat_observe': "チャットを確認中",
  ...
  'tool.staged_screenshot': 'スクリーンショットを説明中',
  'tool.done': '仕上げ中',
  ...
  "st.vision.local.testing": "ローカルビジョンを読み込んでテストしています…",
  ...
  'tr.lineage.standalone': '単独実行',
  'tr.lineage.incomplete': '関連する実行の一部が読み込み済みリストの外にある可能性があります。',
  'tr.lineage.toggle': '子実行を展開または折りたたむ',
  'tr.lineage.missing_parent': '親実行が読み込み済みリストにありません。',
  'tr.lineage.ambiguous_parent': '親リンクが曖昧なため、この実行はルートのままです。',
  'tr.lineage.cross_session_parent': '親が別セッションに属するため、この実行はルートのままです。',
  'tr.lineage.duplicate_id': '実行 ID が重複しているため、この実行はルートのままです。',
  'tr.lineage.cycle': '循環を検出したため、この実行はルートのままです。',
  ...
  "st.transcription.heading": "音声からテキストへ",
  ...
  'st.display.cloud_bridge.desc': 'ポート 17374 を使用して、この Chromium プロファイルにローカルコントローラーを1つ接続します。有効にできるブリッジは一度に1つだけで、標準の権限確認は引き続き適用されます。',
  ...
  'st.display.always_allow_api_mutations.desc': "会話ごとに /allow-api を必要とせず、WebBrain が fetch_url または research_url で POST、PUT、PATCH、DELETE を使用できるようにします。UI優先の指針と確認チェックは引き続き適用されます。既定ではオンです。",
  'st.display.api_mutation_observer.label': 'API変更オブザーバー',
  'st.display.api_mutation_observer.desc': '同じタブの XHR/fetch リクエスト URL とメソッドを監視し、WebBrain が繰り返しの UI アクションを検出して API ショートカットパターンを提案できるようにします。既定ではオンです。',
  'st.display.openai_ask_streaming.label': "Ask モードの応答をストリーミング",
  'st.display.openai_ask_streaming.desc': "対応プロバイダーの Ask モードで、テキストが届いたら表示します。中断されたストリームは通知を表示し、ストリーミングなしで一度だけ再試行します。プロバイダー/API エラーは引き続き表示されます。ツール呼び出しはストリームの完了まで待機し、Act・Dev・スケジュール・クラウド・Continue の実行はストリーミングされません。デフォルトでオン。",
  ...
  "st.display.help_improve.desc_html": "選択された WebBrain Compass の操作を保持し、評価、改善、微調整、トレーニングに使用することを許可します。既定でオンです。今後の Compass 操作がこれらの目的に使用されないようにするにはオフにしてください。<u>ローカルモデルやご自身のAPIへのリクエストは、「研究のためにクエリを共有」をオンにしたプロバイダーからのみWebBrainが収集します。</u><a href=\"https://webbrain.one/privacy\" target=\"_blank\" rel=\"noopener noreferrer\" style=\"color:var(--accent);\">プライバシーポリシー →</a>",
  "st.providers.webbrain_data_use.body": "WebBrain Compass には毎日の無料利用枠が含まれます。「WebBrain の改善に協力する」は既定でオンであり、選択された Compass の会話が保持され、評価、改善、微調整、トレーニングに使用される場合があります。今後の Compass 操作をこれらの用途から除外するには、一般 → 詳細設定でオフにしてください。<u>ローカルモデルやご自身のAPIへのリクエストは、このプロバイダーの「研究のためにクエリを共有」オプションをオンにした場合のみWebBrainが収集します。</u>{privacyLink}。追加利用は {subscribeLink} で購読してください。請求は {accountLink} で管理できます。",
  'st.providers.share_research.label': "研究のためにクエリを共有",
  'st.providers.share_research.hint': "このプロバイダーのプロンプトと応答を、使用したプロバイダーとモデル名とともに評価・改善のためWebBrainに送信します。画像やバイナリ添付は共有前に除去され、テキストは切り詰められます。残りのテキストはそのまま送信されます。",
  'st.providers.share_research.confirm': "このプロバイダーのクエリを研究のためにWebBrainと共有しますか?\n\nオンにすると、このプロバイダーとのプロンプト、応答、ツール操作がプロバイダー名・モデル名とともに評価・改善のためWebBrainに送信されます。テキストは画像除去・長い内容の切り詰め後にそのまま送信されるため、機密の個人情報を共有しないでください。オフにすればいつでも今後の共有を停止できます。",
  ...
  "st.sync.lede_html": "<strong>WebBrain Compass メンバーのみ</strong> デバイス間でメモリ、プロファイルの自動入力、API キー プロバイダーの設定を同期します。アップロード前に、すべてが同期パスワードで暗号化されます。 WebBrain はそれを読み取ることも回復することもできません。チャット履歴と OAuth サインインは同期されません。",
  "st.sync.email.label": "WebBrain Compass メール",
  ...
  "st.sync.status.subscription": "暗号化同期には WebBrain Compass メンバーシップが必要です。",
  "st.sync.status.error": "暗号化された同期には注意が必要です。",
  "st.sync.status.auth_required": "暗号化された同期を使用するには、WebBrain Compass メールでサインインします。",
  ...
  "st.sync.validation.email_required": "WebBrain Compass の請求先メールアドレスを入力します。",
  ...
  "st.sync.auth.check_email": "メールを確認し、WebBrain Compass サインイン リンクを承認して、ここに戻ってください。",
