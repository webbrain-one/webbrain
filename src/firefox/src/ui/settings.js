...
function showProfileSyncResult(ok, text) { if (!profileSyncResult) return; profileSyncResult.className = `test-result show ${ok ? 'ok' : 'error'}`; profileSyncResult.textContent = text; }
async function refreshProfileSyncState() { const state = await sendToBackground('profile_sync_state').catch(e => ({ status: 'error', error: e.message })); if (profileSyncStatus) profileSyncStatus.textContent = `Status: ${state.status || 'disabled'}${state.authenticated ? ' · authenticated' : ''}${state.unlocked ? ' · unlocked' : ' · key not in memory'}`; return state; }
async function profileSyncAction(action, data = {}) { try { const result = await sendToBackground(action, data); showProfileSyncResult(true, 'Cloud Sync updated.'); await refreshProfileSyncState(); return result; } catch (error) { showProfileSyncResult(false, error?.message || 'Cloud Sync failed.'); throw error; } }
...
document.getElementById('btn-profile-sync-auth')?.addEventListener('click', async () => { const email = (profileSyncEmail?.value || '').trim(); if (!email) return showProfileSyncResult(false, 'Enter your WebBrain Cloud billing email.'); try { profileSyncChallenge = await profileSyncAction('profile_sync_auth_start', { email }); showProfileSyncResult(true, 'Check your email, approve Cloud Sync, then return here.'); const poll = setInterval(async () => { if (!profileSyncChallenge) return clearInterval(poll); try { const result = await sendToBackground('profile_sync_auth_status', { challengeId: profileSyncChallenge.challenge_id, verifier: profileSyncChallenge.verifier }); if (result.token) { clearInterval(poll); profileSyncChallenge = null; showProfileSyncResult(true, 'Cloud Sync authenticated. Set a password and enable sync.'); await refreshProfileSyncState(); } } catch (error) { clearInterval(poll); profileSyncChallenge = null; showProfileSyncResult(false, error.message); } }, 3000); setTimeout(() => clearInterval(poll), 30 * 60 * 1000); } catch { } });
document.getElementById('btn-profile-sync-enable')?.addEventListener('click', () => { try { profileSyncAction('profile_sync_unlock', { password: checkedSyncPassword(true), create: true }); } catch (e) { showProfileSyncResult(false, e.message); } });
document.getElementById('btn-profile-sync-unlock')?.addEventListener('click', () => { try { profileSyncAction('profile_sync_unlock', { password: checkedSyncPassword(), create: false }); } catch (e) { showProfileSyncResult(false, e.message); } });
document.getElementById('btn-profile-sync-now')?.addEventListener('click', () => profileSyncAction('profile_sync_now'));
document.getElementById('btn-profile-sync-lock')?.addEventListener('click', () => profileSyncAction('profile_sync_lock'));
document.getElementById('btn-profile-sync-change')?.addEventListener('click', () => { const oldPassword = window.prompt('Current sync password:'); if (!oldPassword) return; try { profileSyncAction('profile_sync_change_password', { oldPassword, newPassword: checkedSyncPassword(true) }); } catch (e) { showProfileSyncResult(false, e.message); } });
document.getElementById('btn-profile-sync-disable')?.addEventListener('click', () => { if (window.confirm('Disable Cloud Sync on this device? Local data will remain.')) profileSyncAction('profile_sync_disable'); });
document.getElementById('btn-profile-sync-reset')?.addEventListener('click', () => { if (!window.confirm('Permanently delete the encrypted cloud vault and replace it with this device’s local data?')) return; try { profileSyncAction('profile_sync_reset', { password: checkedSyncPassword(true) }); } catch (e) { showProfileSyncResult(false, e.message); } });
