...
// bootstrap from inline <head> scripts) and browser.storage.local (canonical
// source the settings page reads/writes). browser.storage.onChanged keeps
...
/** Pull the canonical mode from browser.storage.local, fall back to local. */
...
export function watch(getMode) {
  ...
  return () => {
    ...
    if (onStorage && globalThis.browser?.storage?.onChanged) {
