...
function trustedBuiltInSkillTools(skillId, sourceType, sourceUrl) {
  if (skillId !== 'chrome-web-store-release' || sourceType !== 'built-in' || sourceUrl !== 'skills/chrome-web-store-release.md') return [];
  const shared = {
    kind: 'chromeWebStore', endpoint: 'https://chromewebstore.googleapis.com/', credentials: 'oauth',
    requiresDownloadPermission: false, requiresUploadPermission: false, defaultArgs: {}, activeTabUrlArg: '', inputUrlArg: '',
    allowedInputUrls: [], resultPolicy: 'untrusted', responseLimits: {}, siteAdapters: [], job: {}, tiers: ['full', 'mid'],
  };
  return [
    {
      ...shared, id: 'chrome_web_store_status', name: 'chrome_web_store_status',
      ...
      method: 'GET', readOnly: true, modes: ['ask', 'act', 'dev'],
      parameters: { type: 'object', properties: {}, required: [] },
    },
    {
      ...shared, id: 'chrome_web_store_upload', name: 'chrome_web_store_upload',
      ...
      method: 'POST', readOnly: false, requiresUploadPermission: true, modes: ['act', 'dev'],
      parameters: { type: 'object', properties: {}, required: [] },
    },
    {
      ...shared, id: 'chrome_web_store_publish', name: 'chrome_web_store_publish',
      ...
      method: 'POST', readOnly: false, modes: ['act', 'dev'],
      parameters: { type: 'object', properties: {
        publish_type: { type: 'string', enum: ['default', 'staged'], description: 'default publishes after approval; staged waits for a later manual publish after approval.' },
        deploy_percentage: { type: 'integer', minimum: 0, maximum: 100, description: 'Optional initial rollout percentage. Omit to preserve the dashboard setting.' },
      }, required: [] },
    },
  ];
}
