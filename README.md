...
- **Ask streaming** — Interactive Ask chats show text as it arrives for supported OpenAI, Anthropic, Azure OpenAI, Gemini, DeepSeek, xAI, Mistral, Nvidia NIM, Groq, Alibaba Cloud, Together AI, Fireworks, z.ai, OpenRouter, WebBrain Cloud, llama.cpp, Ollama, LM Studio, Jan, vLLM, SGLang, and LocalAI requests; tools and history wait for the provider's terminal event, with an Advanced kill switch and one silent non-streaming fallback for interrupted streams
