...
- **WebMCP Fast Path (experimental)** — On supporting Chrome pages, Mid/Full and Ask runs can discover page-declared structured tools through CDP; Act/Dev can invoke them by opaque ID instead of guessing DOM controls. Catalogs, annotations, and results stay inside the untrusted-page boundary, and every invocation uses the normal permission gate.
...
WebMCP annotations such as `readOnly` are page-authored hints, not a security
